...
def problem1(square, thickness, window):
    ...
    circle = rg.Circle(rg.Point(x1, y1), (square.length_of_each_side / 2))
    circle.fill_color = square.fill_color
    circle.outline_thickness = thickness
    circle.attach_to(window)
    end_x = square.get_bounding_box().get_upper_left_corner().x
    end_y = square.get_bounding_box().get_upper_left_corner().y + (square.length_of_each_side / 2)
    end = rg.Point(end_x, end_y)
    line = rg.Line(circle.center, end)
