...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(250, 100),  # All numbers are approximate.
                rg.Point(200, 150),
                rg.Point(150, 100),
                rg.Point(200, 50)]
...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window, circle, 4, 'cyan')
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(endpoints)):
        line = rg.Line(start_point, endpoints[k])
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 9, 3, 'red', 6)
    window.close_on_mouse_click()
