...
def main():
    ...
    better_draw_circles(rg.Point(20,20), 3)
    better_draw_circles(rg.Point(-100, 100), 10)
    better_draw_circles(rg.Point(-200,-150), 15)

    even_better_draw_circles(rg.Point(50, 50), 50, 3, "red", 3)
    even_better_draw_circles(rg.Point(150, -150), 100, 5, "blue", 5)
    even_better_draw_circles(rg.Point(-150, 150), 70, 7, "green", 8)
...
###############################################################################
def better_draw_circles(point, radiiChange):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN x pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(radiiChange)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(radiiChange * k)  # Radius 15, 30, 45, 60, ...
...
###############################################################################
def even_better_draw_circles(point, radiiChange, circleCount, circleColor, circleThickness):
    ...
    turtle = rg.SimpleTurtle()
    turtle.pen.color = circleColor
    turtle.pen.thickness = circleThickness
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(1, circleCount+1):  # k becomes 1, 2, 3, ... 10
