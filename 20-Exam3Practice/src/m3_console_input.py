...
def input_it_all():
    ...
    Here is another sample run, with different inputs:
      ...
      This example returns the string
           "4 3.48 7.48 0.93 Peace & LovePeace & LovePeace & LovePeace & Love"
    """
    num_float = input("Please enter a positive floating point number: ")
    num_int = input("Please enter a positive integer: ")
    string = input("Please enter a string: ")
    summed = int(num_int) + float(num_float)
    sin = math.sin(int(num_int) + float(num_float))
    sin = round(sin, 2)
    nums = [num_int, num_float, summed, sin]
    response = ''
    for k in range(len(nums)):
        response = response + str(nums[k])
        response = response + " "
    for j in range(int(num_int)):
